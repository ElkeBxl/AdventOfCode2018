...
(define (find-cart-idx cart carts start-idx)
  (if (= start-idx (vector-length carts))
      #f
      (let ([search-cart (vector-ref carts start-idx)])
        (if (and (= (get-position-x search-cart) (get-position-x cart))
                 (= (get-position-y search-cart) (get-position-y cart)))
            start-idx
            (find-cart-idx cart carts (+ start-idx 1))))))
...
(define (sort-carts carts)
  (vector-sort carts string<? #:key (lambda (cart) (string-append (string (integer->char (get-position-y cart))) "x" (string (integer->char (get-position-x cart)))))))
  ...
  (let loop ([carts (sort-carts carts)])
    (let inner-loop ([idx 0]
      ...
      (if (= idx (vector-length carts))
          ...
          (if (= (vector-length carts) 1)
              (let* ([cart-to-move (vector-ref carts 0)]
                ...
                (move (modulo idx (vector-length carts)) cart-to-move current-direction))
              (let* ([cart-to-move (vector-ref carts (modulo idx (vector-length carts)))]
                ...
                (let ([collided (find-collision carts)])
                  (if collided
                      (let ([c (vector-ref carts collided)])
                        (if (< collided idx)
                            (set! idx (- idx 1))
                            #f)
                        (set! carts (vector-append (vector-take carts collided) (vector-drop carts (+ collided 1))))
                        (let ([c-idx (find-cart-idx c carts collided)])
                          (set! carts (vector-append (vector-take carts c-idx) (vector-drop carts (+ c-idx 1))))
                          (if (< c-idx idx)
                              (set! idx (- idx 1))
                              #f)
                          (inner-loop idx good-moves)))
                      (inner-loop (+ idx 1) (+ good-moves 1))))))))))
